<div class="row g-4">
    <div class="col-lg-5">
        <div class="card border-0 shadow-sm">
            ...
            <div class="card-body">
                <form method="POST" action="{{ route('admin.users.notes.store', $user) }}">
                    @csrf
                    <div class="mb-2">
                        <textarea name="note" class="form-control" rows="5" required placeholder="Internal admin note on this user (only visible to admins)..."></textarea>
                    </div>
                    <div class="form-check mb-3">
                        <input type="checkbox" name="pinned" value="1" class="form-check-input" id="userPinNoteCheck">
                        <label class="form-check-label small" for="userPinNoteCheck">Pin this note to the top</label>
                    </div>
    ...
    <div class="col-lg-7">
        <div class="card border-0 shadow-sm">
            <div class="card-header bg-white py-3">
                <h6 class="fw-bold mb-0"><i class="bi bi-journal-text me-2"></i>All Admin Notes ({{ $user->adminNotes->count() }})</h6>
            </div>
                    ...
                    @foreach($user->adminNotes as $note)
                        <div class="border-bottom p-3 {{ $note->pinned ? 'bg-warning bg-opacity-10' : '' }}">
                            ...
                            <div class="text-end mt-2">
                                <form method="POST" action="{{ route('admin.users.notes.toggle-pin', $note) }}" class="d-inline">
                                    @csrf @method('PATCH')
                                    <button class="btn btn-outline-warning btn-sm py-0 px-2" title="{{ $note->pinned ? 'Unpin' : 'Pin' }}">
                                        <i class="bi bi-pin{{ $note->pinned ? '' : '-angle' }}"></i>
                                    </button>
                                </form>
                                <form method="POST" action="{{ route('admin.users.notes.destroy', $note) }}" class="d-inline" onsubmit="return confirm('Delete this note?');">
                                    @csrf @method('DELETE')
